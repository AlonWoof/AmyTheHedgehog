...
Shader "InfiniGRASS/Vertex Grass Spec Detailed Wind Deffered" {
    Properties {
        ...
        _RippleScale ("Vary Height", Float ) = 0
           ...
           _LightDir ("_LightDir", Float) = (0.5,0.5,0.5,0)
             _LightColor ("LightColor", Color) = (0.5,0.8,0.5,0) //0.5,0.8,0.5
    }
    SubShader {
        Tags {
            //"Queue"="Opaque"
            "RenderType"="Opaque+1" //+1 SSAO PRO COMP
        }
            ...
            void frag(
                ...
                float facing : VFACE)
              {
               ...
               float dist = distance(_OceanCenter, float3(_WaveControl1.x*i.posWorld.y,_WaveControl1.y*i.posWorld.x,_WaveControl1.z*i.posWorld.z) );
                ...
                float3 finalColor = 0;
                float3 diffuseLight = diffuseC;//diffuse;
                //float node_331 = 1.0;
				...
				if(i.uv0.y >= 1-(3 * (_SnowCoverage+_TimeControl1.y+normalLocal.r)) * col.r*normalLocal.b*(normalLocal.r+0.4)+0.001)
	            { 	                  
	                //col.rgb = (float4(i.uv0.y,i.uv0.y,i.uv0.y,1)*4*+_TimeControl1.z)*(finalColor)*8;  	
	                col.rgb = (float4(i.uv0.y,i.uv0.y,i.uv0.y,1)*4*+_TimeControl1.z)*8;                                     
	            } 
                ...
                outSpecSmoothness = half4(specular*_SpecularPower*1, gloss/5);
                ...
                //outNormal = half4(specular*_SpecularPower*1+normalDirection*1,1);
                //outNormal = half4(specular*_SpecularPower*_LightColor.w-normalDirection,1);
                //outNormal = half4(specular*_SpecularPower*_LightColor.w*normalDirection*1-normalDirection*0.5,1);
                // outNormal = half4(specular*_SpecularPower*_LightColor.w-normalDirection*0.5,1);
                //outNormal = half4(specular*_SpecularPower*_LightColor.w-NdotL*normalDirection*3.5,1);
                // outNormal = half4(specular*_SpecularPower*_LightColor.w-1*normalDirection*3.5,1);
                ...
                // outEmission = half4((finalColor/1+col/465)*_LightColor.w,1);
                outEmission = half4((finalColor/2+col/465)*_LightColor.w*1,1);
                #ifndef UNITY_HDR_ON
              ...
              finalColor = lerp(finalColor, finalColor*_Color,_TintPower*i.uv0.y*(0.9+0.6*cos(i.posWorld.x*2*_TintFrequency)+0.6*sin(i.posWorld.z*3*_TintFrequency)+0.6*sin(i.posWorld.z*1*_TintFrequency+0.1)));
               ...
               //SNOW
                float3 col = finalColor;                
               	// float4 SnowTexColor = tex2D(_SnowTexture,  i.uv0);				
				//if(i.uv0.y >= 1-(3 * (_SnowCoverage+_TimeControl1.y-1)) * col.r* col.r* col.r* 1+0.01)
				if(i.uv0.y >= 1-(3 * (_SnowCoverage+_TimeControl1.y+normalLocal.r-1)) * col.r*normalLocal.b*(normalLocal.r+0.4)+0.001)
	            { 	                  
	                //col.rgb = (float4(i.uv0.y,i.uv0.y,i.uv0.y,1)*4*+_TimeControl1.z)*(finalColor)*8;  	
	                col.rgb = (float4(i.uv0.y,i.uv0.y,i.uv0.y,1)*4*+_TimeControl1.z)*8;                                     
	            } 
...
//        }
        Pass {
            ...
            Cull Off //REMOVE for SSAO PRO COMP
            Offset 1, 1
