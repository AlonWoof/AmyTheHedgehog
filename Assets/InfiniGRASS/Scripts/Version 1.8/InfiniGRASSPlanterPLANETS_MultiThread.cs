...
public class InfiniGRASSPlanterPLANETS_MultiThread : MonoBehaviour {

        //v2.1.4 - Planets growing sample
        public List<GameObject> Planets = new List<GameObject>();
        public List<int> PlanetsGrown = new List<int>();
        public float plantPlanetDistance = 500;
        public float plantPlanetStrokes = 20;
        ...
        //v2.1.5 - force radial direction all brushes
        public bool forceRadialDirection = false;
	...
	void Start () {
        ...
        //2.1.4 - Planets
        private IEnumerator coroutinePlantPlanets;
        IEnumerator plantPlanet(Vector3 planetPos, float planetScale) //plant with plantPlanetStrokes number
        {
            float sqrtAdder = Mathf.Sqrt(plantPlanetStrokes);
            int adder = (int)(360 / sqrtAdder);
            //Debug.Log("ADDER = " + adder + " sqrt = " + sqrtAdder);
            for (int i=0; i < Mathf.Sqrt(plantPlanetStrokes) * 360; i = i + adder)
            {
                //Debug.Log("i = " + i);
                for (int j = 0; j < Mathf.Sqrt(plantPlanetStrokes) * 360; j = j + adder)
                {
                    //Debug.Log("j = " + j);
                    //Find random position in planet and plant it
                    //plant on it
                    //https://en.wikipedia.org/wiki/Spherical_coordinate_system
                    float covertToRad = Mathf.PI / 180;
                    float coordX = planetScale * Mathf.Cos(i * covertToRad) * Mathf.Sin(j * covertToRad);//i = phi, j=theta
                    float coordY = planetScale * Mathf.Sin(i * covertToRad) * Mathf.Sin(j * covertToRad);
                    float coordZ = planetScale * Mathf.Cos(j * covertToRad);
                    Vector3 rayStart = new Vector3(coordX, coordZ,coordY ) * 1.05f + planetPos;
                    Ray ray = new Ray();
                    ray.origin = rayStart;// new Vector3(tilePos.x, raycastHeight, tilePos.z);
                    ray.direction = planetPos - rayStart;// - Vector3.up;
                    RaycastHit hit = new RaycastHit();

                    //Debug.Log(rayStart);
                    //Debug.DrawLine(rayStart, rayStart + ray.direction * 100, Color.red, 8);
                    //Debug.DrawLine(rayStart, rayStart - ray.direction * 100, Color.blue, 8);
        ...
        void Update()
        {
            ...
            if(Planets.Count > 0)
            {
                for (int i=0;i < Planets.Count; i++)
                {
                    if (Vector3.Distance( Planets[i].transform.position, Camera.main.transform.position) < plantPlanetDistance )
                    {
                        if (PlanetsGrown[i] == 0)
                        {
                            coroutinePlantPlanets = plantPlanet(Planets[i].transform.position, Planets[i].transform.localScale.x);
                            StartCoroutine(coroutinePlantPlanets);                            
                            PlanetsGrown[i] = 1;
                            //Debug.Log("Growing planet" + i);
                        }
                    }
                }
            }
...
//					TREE.maxOutofPlaneSlopeHeight = Grassmanager.maxOutofPlaneSlopeHeight;
                    ...
                    //v2.1.5
                    if (forceRadialDirection)
                    {
                        TREE.customRot = false;
                        TREE.RotTowards = false;
                    }
