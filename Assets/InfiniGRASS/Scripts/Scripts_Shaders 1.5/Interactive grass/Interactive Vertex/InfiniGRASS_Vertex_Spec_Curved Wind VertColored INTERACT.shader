...
Shader "InfiniGRASS/Vertex Grass Spec Curved Wind VertColored INTERACT" {
    Properties {
        ...
        _RippleScale ("Vary Height", Float ) = 0
           ...
           _WaveYFactor("Wave Control y axis", Float) = 1
			...
			_tintOffset("_tintOffset", Float) = 0
			_vertexColorPower("_vertexColorPower", Float) = 0
    }
    SubShader {
        Tags {
            //"Queue"="Opaque"
...
//						}
	           ...
	           _BulgeScale= _BulgeScale* _BulgeScale_copy;
				...
				//dist = 100 * _BulgeShape;
				  dist =SpeedFac *  90* (cos(_BulgeShape+_Time.y/15))-_SmoothMotionFactor;
				///////////////////////// 
				if( o.uv0.y > 0.1){
				...
				if( o.uv0.y > _HeightControlPoints.z){
					...
					o.posWorld.x += 1 * _BulgeScale*4*cos(o.posWorld.x*_WaveControl1.x+_Time.y*_TimeControl1.x + o.posWorld.z*_WaveControl1.z)*0.1*sin(o.posWorld.z+_Time.y) + _WaveXFactor*((2+cos(o.posWorld.x/dist))*_OceanCenter.x/2) + _WaveYFactor*((3+sin(2*o.posWorld.z/dist))*_OceanCenter.z/2);
					o.posWorld.z += 1 * _BulgeScale*4*sin(o.posWorld.x*_WaveControl1.x+_Time.y*_TimeControl1.x + o.posWorld.z*_WaveControl1.z)*0.1*cos(o.posWorld.z+_Time.y) + _WaveXFactor*((2+sin(o.posWorld.z/dist))*_OceanCenter.z/2) + _WaveYFactor*((3+cos(3*o.posWorld.x/dist))*_OceanCenter.x/2);	
				}		
				if( o.uv0.y > _HeightControlPoints.w){
					
					o.posWorld.x +=  _BulgeScale*15*cos(o.posWorld.x*_WaveControl1.x+_Time.y*_TimeControl1.x + o.posWorld.z*_WaveControl1.z)*0.1*sin(o.posWorld.z+_Time.y) + _WaveXFactor*((2+cos(o.posWorld.x/dist))*_OceanCenter.x/0.9)	+ _WaveYFactor*((3+sin(2*o.posWorld.z/dist))*_OceanCenter.z/1);
					o.posWorld.z += _BulgeScale*15*sin(o.posWorld.x*_WaveControl1.x+_Time.y*_TimeControl1.x + o.posWorld.z*_WaveControl1.z)*0.1*cos(o.posWorld.z+_Time.y) + _WaveXFactor*((2+sin(o.posWorld.z/dist))*_OceanCenter.z/0.9) + _WaveYFactor*((3+cos(3*o.posWorld.x/dist))*_OceanCenter.x/1);
				}	
				
				//GRAVITY
				if( o.uv0.y > 0.5){
					o.posWorld.y = o.posWorld.y - 3*(o.uv0.y-0.5) ;//+_BulgeScale*0.5*cos(o.posWorld.x*_WaveControl1.x+_Time.y*_TimeControl1.x + o.posWorld.z*_WaveControl1.z)*0.1*sin(o.posWorld.z+_Time.y) ;
				}
...
specular *= ((attenuation) * _LightColor0.xyz);////////////// INFINIGRASS FIX !!!! - fixes flickering shadows - disable all above
			...
			//	i.vertexColor.rgb = i.vertexColor.rgb * 5;


              	specular = specular * (i.uv0.y*2-0.5) ;              
              	
              	float3 colors = lerp(finalColor, i.vertexColor.rgb+((1-i.uv0.y)*_BaseLight*i.vertexColor.rgb), 1-(i.uv0.y*_BaseColorYShift));
              
            //   finalColor = lerp(colors, finalColor* i.vertexColor*_Color ,_TintPower*i.uv0.y*(0.9+0.6*cos(i.posWorld.x*0.2)+0.6*sin(i.posWorld.z*0.3)+0.6*sin(i.posWorld.z*0.1+0.1)));
            ...
            //   finalColor = lerp(finalColor, finalColor*_Color,_TintPower*i.uv0.y*(0.9+0.6*cos(i.posWorld.x*2*_TintFrequency)+0.6*sin(i.posWorld.z*3*_TintFrequency)+0.6*sin(i.posWorld.z*1*_TintFrequency+0.1)));
				...
				//float _vertexColorPower;
               // finalColor = lerp(colors, finalColor* i.vertexColor*_Color,_TintPower*i.uv0.y*(0.9+0.6*cos(i.posWorld.x*2*_TintFrequency)+0.6*sin(i.posWorld.z*3*_TintFrequency)+0.6*sin(i.posWorld.z*1*_TintFrequency+0.1)));
				//finalColor = lerp(colors, finalColor* i.vertexColor*_Color, _TintPower*i.uv0.y*(0.9 + 0.6*cos(i.posWorld.x * 2 * _TintFrequency) + 0.6*sin(i.posWorld.z * 3 * _TintFrequency) + 0.6*sin(i.posWorld.z * 1 * _TintFrequency + 0.1)));
				finalColor = finalColor * (1 - _vertexColorPower) +
					_vertexColorPower*lerp(
						colors, 
						finalColor* i.vertexColor*_Color, 
						(_TintPower + _tintOffset)*i.uv0.y*(0.9 + 0.6*cos(i.posWorld.x * 2 * _TintFrequency)
							+ 0.6*sin(i.posWorld.z * 3 * _TintFrequency) 
							+ 0.6*sin(i.posWorld.z * 1 * _TintFrequency + 0.1)
							));

               finalColor += specular* _SpecularPower;

               //v1.7.8f
               finalColor = lerp(finalColor, finalColor*_Color, (_TintPower + _tintOffset)*i.uv0.y*(0.9+0.6*cos(i.posWorld.x*2*_TintFrequency)+0.6*sin(i.posWorld.z*3*_TintFrequency)+0.6*sin(i.posWorld.z*1*_TintFrequency+0.1)));
               
/// Final Color:
 UNITY_APPLY_FOG(i.fogCoord, finalColor); //v1.7.7

 //return float4(1,0,0, 1);
 //return float4(i.vertexColor.rgb*1,1);
            ...
            VertexOutput vert (VertexInput v) {
              ...
              //  v.vertex.xyz += (normalize((float3(1,0.5,0.5)+v.normal))*node_389.r*sin(((node_389.b*3.141592654)+node_392.g+node_392.b))*0.16);
               ...
               // if( distance(_InteractPos.x,o.posWorld.x) < _StopMotionThreshold/2 ){    
                	...
                	SpeedFac =  _InteractSpeed * _WaveControl1.w; 
                	//if( o.uv0.y > 0.2){
						//	o.posWorld.x += (_InteractSpeed.x*2+0.1)*(cos(o.posWorld.x*_WaveControl1.x+_Time.y*_TimeControl1.x + o.posWorld.z*_WaveControl1.z)*0.1*sin(o.posWorld.z+_Time.y) + _WaveXFactor*((2+cos(o.posWorld.x/dist))*_OceanCenter.x/5) + _WaveYFactor*((3+sin(2*o.posWorld.z/dist))*_OceanCenter.z/5));
						//	o.posWorld.z += (_InteractSpeed.z+0.1)*sin(o.posWorld.x*_WaveControl1.x+_Time.y*_TimeControl1.x + o.posWorld.z*_WaveControl1.z)*0.1*cos(o.posWorld.z+_Time.y) + _WaveXFactor*((2+sin(o.posWorld.z/dist))*_OceanCenter.z/5) + _WaveYFactor*((3+cos(3*o.posWorld.x/dist))*_OceanCenter.x/6);
						//}
						if( o.uv0.y < 0.3){
						//_WaveXFactor = _WaveXFactor - (1-distA)*(1-distA)*SpeedFac.z;
