...
Shader "InfiniGRASS/InfiniGrass Directional Wind SNOW TERRAIN BLEND" {
    Properties {
        ...
        _OceanCenter("Ocean Center", Vector) = (0, 0, 0, 0)
         ...
         _RippleScale ("Vary Height", Float ) = 0     
		   ...
		   //BLEND
		   _TerrainTexture("_TerrainTexture", 2D) = "white" {}
		   _TopDownCameraHeight("Top Down Camera Height", Float) = 100
		   _TerrainScale("Terrain Scale", Float) = 1000
		   _TerrainBlendParams("Terrain Blend Height - Exponent - Noise", Float) = (1,1,1,1)
    }
    SubShader {
        Tags {
            "Queue"="AlphaTest"
        ...
        Pass {
           ...
           // #define UNITY_PASS_FORWARDBASE
			...
			//BLEND
			uniform sampler2D _TerrainTexture; uniform float4 _TerrainTexture_ST;
			float _TopDownCameraHeight;
			float _TerrainScale;
			float4 _TerrainBlendParams;
...
/// Final Color:
				...
				//BLEND
				float4 TerrainTexColor = tex2D(_TerrainTexture, (i.posWorld.xz / _TerrainScale)+float2(0.5,0.5) );
				//col =  TerrainTexColor * 0.4; //_TerrainBlendParams
				//col = lerp(TerrainTexColor,col, ((_TerrainBlendParams.x - i.posWorld.y)*(_TerrainBlendParams.x - i.posWorld.y) / _TerrainBlendParams.y)  * _TerrainBlendParams.z);
				//col = lerp(col, TerrainTexColor, saturate((_TerrainBlendParams.x - i.pos.y)*(_TerrainBlendParams.x - i.pos.y) / _TerrainBlendParams.y)  * _TerrainBlendParams.z);
				//col = lerp(col, TerrainTexColor, saturate( (_TerrainBlendParams.y-i.pos.y )*(_TerrainBlendParams.y - i.pos.y) / _TerrainBlendParams.x));
				//col = lerp(col, TerrainTexColor, saturate(i.pos.y * _TerrainBlendParams.x));
				/*if (i.posWorld.y - i.origin.y < _TerrainBlendParams.x) {
					col = TerrainTexColor;
				}*/
				col = lerp(col, TerrainTexColor*0.5* col + TerrainTexColor*0.5, 1-saturate(   pow((i.posWorld.y - i.origin.y), _TerrainBlendParams.y) - _TerrainBlendParams.x));
...
////// Specular:
				...
				finalColor += diffuseLight * (node_1.rgb)*_ColorGlobal1 * 5 * attenuation; //v1.9.6
